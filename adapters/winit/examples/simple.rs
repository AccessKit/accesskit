...
impl WindowState {
    fn new(window: Arc<Window>, adapter: Adapter, ui: UiState) -> Self {
        Self {
...
impl ApplicationHandler<AccessKitEvent> for Application {
    fn window_event(&mut self, _: &ActiveEventLoop, _: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::Resized(_) => {
                ...
                window.ui.set_viewport(scale_factor, inset);
    ...
    fn user_event(&mut self, _: &ActiveEventLoop, user_event: AccessKitEvent) {
        ...
        match user_event.window_event {
            ...
            AccessKitWindowEvent::ActionRequested(request) => {
                window.ui.do_action(&request);
                window.update_accessibility_tree();
                window.window.request_redraw();
            }
            AccessKitWindowEvent::AccessibilityDeactivated => {
                window.ui.deactivated();
            }
        }
    }
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if util::flush_announcement(event_loop, &mut window.ui) {
            window.update_accessibility_tree();
        }
    }
}

...
use accesskit::{
    ActionHandler, Live, NodeId, Role, TreeId, TreeInfo, TreeUpdate as TreeUpdateTrait,
};
use accesskit_consumer::{
    FilterResult, FullNodeId, NodeRef, NonGenericActivationHandler, Tree, TreeChangeHandler,
    TreeState, TreeUpdate,
};
...
impl<'a, 'b> AdapterChangeHandler<'a, 'b> {
    fn new(context: &'a Arc<Context>, event_ctx: &'b mut EventContext) -> Self {
        Self {
            context,
        ...
        match &self.state {
            ...
            State::Placeholder(context) => {
                ...
                *context.tree.write().unwrap() = tree;
                ...
                let result = context.read_tree().state().focus().map(|node| {
                    let queue = event_ctx.write_queue();
                    queue.push(focus_event(context, node.id()));
                    QueuedEvents(queue)
                });
            ...
            State::Active(context) => {
                let mut handler = AdapterChangeHandler::new(context, event_ctx);

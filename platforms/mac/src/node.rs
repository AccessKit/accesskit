...
use accesskit::{NodeId, Role, Tree};
...
fn get_identifier(_state: &State, node: &Node) -> id {
    make_nsstring(&node.id().0.to_string())
}
...
fn get_title(_state: &State, node: &Node) -> id {
    ...
    make_nsstring(node.name().unwrap_or("".to_string()).as_str())
}
...
impl PlatformNode {
    pub(crate) fn get_or_create(node: &Node, view: &StrongPtr) -> StrongPtr {
        ...
        let key = (node.tree_reader.root().id().clone(), node.id());
...
impl PartialEq for StrongPtr {}
impl Eq for StrongPtr {}
...
lazy_static! {
    ...
    static ref PLATFORM_NODES: Mutex<HashMap<(Tree, NodeId), PlatformNodePtr>> = Mutex::new(HashMap::new());
}

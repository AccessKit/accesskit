...
impl QueuedEvents {
    ...
    ///
    /// The `callback_class` parameter refers to a Java class with methods
    /// that actually send the events. The reference implementation of the
    /// contract for this class is `dev.accesskit.android.Delegate`, the
    /// source code for which is in the `java` directory of this crate.
    /// The methods of that class that are called by this function are all
    /// marked `public static` and have names starting with `send`.
    ///
    /// Whether it is safe to call this function on a thread other than the
    /// UI thread depends on the implementation of the `send` methods in
    /// `callback_class`. If these methods dispatch events asynchronously
    /// to the UI thread, as the implementations in
    /// `dev.accesskit.android.Delegate` do, then it is safe to call this
    /// function on any thread without concern for reentrancy. If they
    /// synchronously call the Android framework to send the events,
    /// then this function must be called on the UI thread, while not holding
    /// any locks required by the host view's implementations of Android
    /// framework callbacks.
    ///
    /// The `host` paramter is the Android view for the adapter that

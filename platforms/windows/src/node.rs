...
impl PlatformNode {
    ...
    where
        for<'a> F:
            FnOnce(Node<'a>, LocalNodeId, TreeId, &Arc<Context>) -> Result<Option<ActionRequest>>,
    {
        let context = self.upgrade_context()?;
        ...
        if let Some(request) = f(node, target_node, target_tree, &context)? {
            drop(tree);
    ...
    {
        self.do_complex_action_with_context(|node, target_node, target_tree, _| {
            f(node, target_node, target_tree)
        })
    }
    ...
    fn set_selected(&self, selected: bool) -> Result<()> {
        self.do_complex_action_with_context(|node, target_node, target_tree, context| {
            if node.is_disabled() {
                return Err(element_not_enabled());
            }

...
impl PlatformNode {
    pub(crate) fn new(context: &Arc<Context>, node_id: FullNodeId) -> Self {
        Self {
    ...
    where
        for<'a> F: FnOnce(NodeRef<'a>, &Context) -> Result<T>,
    {
    ...
    where
        for<'a> F: FnOnce(NodeRef<'a>, &TreeState, &Context) -> Result<T>,
    {
        self.with_tree_and_context(|tree, context| {
            let node = self.node(tree)?;
    ...
    fn relative(&self, node_id: FullNodeId) -> Self {
        Self {

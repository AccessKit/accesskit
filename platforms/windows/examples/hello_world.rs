...
impl WindowState {
    fn get_or_init_accesskit_adapter(&self, window: HWND) -> &accesskit_windows::Adapter {
        self.adapter.get_or_init(|| {
            // Note: For this simple example, the initial tree is mostly static.
            // In a real GUI framework, the initial tree would be dynamically
            // generated by the framework based on application logic and data
            // associated with the window. Here, only the focus is based on
            // window state.
    ...
    fn press_button(&self, window: HWND, id: NodeId) {
        ...
        // of focus changes.
        let adapter = self.get_or_init_accesskit_adapter(window);
        let inner_state = self.inner_state.borrow();
    ...
    match message {
        ...
        WM_KEYDOWN => match VIRTUAL_KEY(wparam.0 as u16) {
            ...
            VK_SPACE => {
                ...
                window_state.press_button(window, id);
                LRESULT(0)
            }

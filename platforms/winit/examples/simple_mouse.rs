...
fn hit_test(x: f64, y: f64) -> Option<NodeId> {
    fn point_in_rect(x: f64, y: f64, rect: &Rect) -> bool {
        x >= rect.x0 && x < rect.x1 && y >= rect.y0 && y < rect.y1
    }

    if point_in_rect(x, y, &BUTTON_1_RECT) {
        Some(BUTTON_1_ID)
    } else if point_in_rect(x, y, &BUTTON_2_RECT) {
        Some(BUTTON_2_ID)
    } else {
        None
    }
    ...
    fn window_event(&mut self, _: &ActiveEventLoop, _: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                fill::fill_window(&window.window, |mut buffer, size| {
                    fill::draw_button(
                        &mut buffer,
                        size,
                        &BUTTON_1_RECT,
            ...
            WindowEvent::MouseInput {
                ...
                if button_state == ElementState::Pressed {
                    if let Some((x, y)) = self.cursor_position {
                        if let Some(hit_id) = hit_test(x, y) {
                            state.set_focus(adapter, hit_id);
                            state.press_button(adapter, hit_id);
                            window.window.request_redraw();
                        }
                    }
                }
...
fn main() -> Result<(), Box<dyn Error>> {
    println!("This example has a basic visible GUI, and a mouse+keyboard interface:");
    ...
    println!();
    println!("Mouse interface:");
    println!("  - Click on a button to focus and press it");

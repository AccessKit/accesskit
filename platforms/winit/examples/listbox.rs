use accesskit::{
    Action, ActionRequest, ActivationHandler, Node, NodeId, Rect, Role, Tree, TreeUpdate,
};
...
fn build_listbox(label: &str) -> Node {
...
impl UiState {
    ...
    fn build_initial_tree(&mut self) -> TreeUpdate {
        ...
        let mut result = TreeUpdate {
            ...
            tree: Some(tree),
            focus: ITEMS[self.focused_item_index].0,
        };
        for i in 0..ITEMS.len() {
            let id = ITEMS[i].0;
            let is_selected = self.selected_items[i];
            result.nodes.push((id, build_option(i, is_selected)));
        }
        result
    }
        ...
        match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                Key::Named(winit::keyboard::NamedKey::ArrowDown) => {
                    ...
                    let new_focused_item_index = match state.focused_item_index {
                        index if index + 1 < ITEMS.len() => index + 1,
                        _ => ITEMS.len() - 1,
                    };
                    state.focus_option(adapter, new_focused_item_index);
                }

...
const HEADING_1_RECT: Rect = Rect {
    x0: 220.0,
    y0: 20.0,
    x1: 420.0,
    y1: 64.0,
...
const HEADING_2_RECT: Rect = Rect {
    x0: 220.0,
    y0: 20.0,
    x1: 420.0,
    y1: 64.0,
...
fn build_document() -> Node {
    ...
    node.set_children(vec![HEADING_1_ID, BUTTON_1_ID, BUTTON_2_ID, HEADING_2_ID]);
    node
}
...
fn build_heading(id: NodeId, label: &str) -> Node {
    let rect = match id {
        HEADING_1_ID => HEADING_1_RECT,
        HEADING_2_ID => HEADING_2_RECT,
        _ => unreachable!(),
    };
...
impl UiState {
    ...
    fn build_root(&mut self) -> Node {
        ...
        node.set_children(vec![DOCUMENT_ID]);
        ...
        match event {
            ...
            WindowEvent::KeyboardInput {
                ...
                Key::Named(winit::keyboard::NamedKey::Tab) => {
                    let new_focus = match state.focus {
                        HEADING_1_ID => BUTTON_1_ID,
                        BUTTON_1_ID => BUTTON_2_ID,
                        BUTTON_2_ID => HEADING_2_ID,
                        HEADING_2_ID => HEADING_1_ID,
                        _ => HEADING_1_ID,
                    };
    ...
    fn user_event(&mut self, _: &ActiveEventLoop, user_event: AccessKitEvent) {
        ...
        match user_event.window_event {
            ...
            AccessKitWindowEvent::ActionRequested(ActionRequest {
                ...
                if
                    target_node == BUTTON_1_ID
                    || target_node == BUTTON_2_ID
                    || target_node == HEADING_1_ID
                    || target_node == HEADING_2_ID
                {
                    match action {
                        Action::Focus => {

...
impl SubclassingAdapter {
    ...
    fn new_internal(
        ...
        let subclass = match subclasses.iter().find(|entry| entry.0 == prev_class) {
            Some(entry) => entry.1,
            None => {
                let mut name_bytes = Vec::from("AccessKitSubclassOf".as_bytes());
                name_bytes.extend_from_slice(prev_class.name().to_bytes_with_nul());
                let name = CStr::from_bytes_with_nul(&name_bytes).unwrap();

...
impl QueuedEvent {
    fn raise(self, tree: &Weak<Tree>, view: &Id<NSView, Shared>, _mtm: MainThreadMarker) {
        match self {
            Self::Generic {
                node_id,
                notification,
            } => {
                let platform_node = PlatformNode::get_or_create(node_id, tree, view);
                unsafe { NSAccessibilityPostNotification(&platform_node, notification) };
            }
            Self::NodeDestroyed(node_id) => {
                if let Some(platform_node) = PlatformNode::remove(node_id, view) {
                    unsafe {
...
impl QueuedEvents {
    ...
    /// required to access the adapter must not be held while this method
    /// is called.
    ///
    /// As indicated by the [`objc2::foundation::MainThreadMarker`]
    /// parameter, this method must be run on the main thread.
    pub fn raise(self, mtm: MainThreadMarker) {
        for event in self.events {
            event.raise(&self.tree, &self.view, mtm);
        }
    }
}
...
impl EventGenerator {
    pub(crate) fn new(tree: Weak<Tree>, view: Id<NSView, Shared>) -> Self {
        Self {
            events: Vec::new(),
            tree,
            view,

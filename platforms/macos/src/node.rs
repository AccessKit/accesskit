...
impl NodeWrapper<'_> {
    ...
    fn is_selectable(&self) -> bool {
        self.0.is_selectable() && self.0.role() != Role::Tab
    }
}
...
declare_class!(
    ...
    unsafe impl PlatformNode {
        ...
        fn is_selected(&self) -> bool {
            self.resolve(|node| {
                let wrapper = NodeWrapper(node);
                wrapper.is_selectable().then(|| node.is_selected()).flatten()
            })
            .flatten()
            .unwrap_or(false)
        }
        ...
        fn set_selected(&self, selected: bool) {
            self.resolve_with_context(|node, context| {
                let wrapper = NodeWrapper(node);
                if !node.is_clickable() || !wrapper.is_selectable() {
                    return;
                }
        ...
        fn pick(&self) -> bool {
            self.resolve_with_context(|node, context| {
                let wrapper = NodeWrapper(node);
                let selectable = node.is_clickable() && wrapper.is_selectable();
                if selectable {
                    context.do_action(ActionRequest {
                        action: Action::Click,
                        target: node.id(),
        ...
        fn is_selector_allowed(&self, selector: Sel) -> bool {
            self.resolve(|node| {
                ...
                {
                    let wrapper = NodeWrapper(node);
                    return node.is_clickable() && wrapper.is_selectable();
                }

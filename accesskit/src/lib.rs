...
impl PropertyValue {
    fn set_string(&mut self, src: &str) {
        match self {
            ...
            _ => {
                *self = PropertyValue::String(src.into());
            }
        }
    }
...
impl Clone for PropertyValue {
    ...
    fn clone_from(&mut self, src: &Self) {
        match src {
            ...
            Self::TreeId(v) => *self = Self::TreeId(*v),
        }
    }
}
        ...
        impl PropertyValue {
            $(fn $method(&mut self, value: $type) {
                *self = match take(self) {
                    ...
                    _ => PropertyValue::$variant(value.into()),
                };
            })*
        }
        impl Node {
        ...
        impl PropertyValue {
            $(fn $setter(&mut self, src: &[$type]) {
                match self {
                    ...
                    _ => {
                        *self = PropertyValue::$variant(src.into());
                    }
                }
            })*
        }
        impl Node {
            ...
            fn $pusher(&mut self, id: PropertyId, item: $type) {
                ...
                match dest {
                    ...
                    _ => {
                        *dest = PropertyValue::$variant(vec![item]);
                    }
                }
            })*
        }
...
macro_rules! f32_property_methods {
    ($($(#[$doc:meta])* ($id:ident, $getter:ident, $setter:ident, $clearer:ident)),+) => {
        $(property_methods! {
            $(#[$doc])*
            ($id, $getter, get_f32, Option<f32>, $setter, set_f32, f32, $clearer)
        })*
        impl Node {
...
macro_rules! color_property_methods {
    ($($(#[$doc:meta])* ($id:ident, $getter:ident, $setter:ident, $clearer:ident)),+) => {
        $(property_methods! {
            $(#[$doc])*
            ($id, $getter, get_color, Option<Color>, $setter, set_color, Color, $clearer)
        })*
        impl Node {
...
impl Clone for Node {
    ...
    fn clone_from(&mut self, src: &Self) {
        ...
        self.child_actions = src.child_actions;
...
length_vec_property_methods! {
    ...
    (WordStarts, word_starts, set_word_starts, push_word_start, clear_word_starts)
}
...
mod tests {
    ...
    #[test]
    fn node_reset_should_clear_everything_but_the_new_role() {
        let mut node = Node::new(Role::Button);
        node.add_action(Action::Click);
        node.add_child_action(Action::ScrollIntoView);
        node.set_hidden();
        node.set_children(&[NodeId(0)]);
        node.reset(Role::CheckBox);
        assert_eq!(node.role(), Role::CheckBox);
        assert!(!node.supports_action(Action::Click));
        assert!(!node.child_supports_action(Action::ScrollIntoView));
        assert!(!node.is_hidden());
        assert!(node.children().is_empty());
    }

    #[test]
    fn node_clone_from_should_be_equivalent_to_clone() {
        let mut src = Node::new(Role::Button);
        src.add_action(Action::Click);
        src.add_child_action(Action::ScrollIntoView);
        src.set_hidden();
        src.set_children(&[NodeId(0)]);
        let expected = src.clone();

        let mut dst = Node::new(Role::Unknown);
        dst.add_action(Action::Focus);
        dst.add_child_action(Action::Blur);
        dst.set_multiselectable();
        dst.set_children(&[NodeId(5), NodeId(6)]);
        dst.clone_from(&src);
